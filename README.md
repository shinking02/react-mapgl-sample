...
3. ZIPファイルを展開
4. **重要**: 展開したフォルダの中の `react-mapgl-sample` フォルダに移動してください
    - 展開直後は `react-mapgl-sample-main` のような名前になっている場合があります
    - ターミナルやコマンドプロンプトで正しいフォルダに移動してください

```bash
# 例：Macの場合
cd ~/Downloads/react-mapgl-sample-main

# 例：Windowsの場合
cd C:\Users\[ユーザー名]\Downloads\react-mapgl-sample-main
```

### 2. VS Code 拡張機能のインストール（推奨）
...
```
react-mapgl-sample/
├── .cursorrules               # Cursor AI エディタ用の設定
...
- **.cursorrules**: Cursor AI エディタ用のプロジェクト固有設定
